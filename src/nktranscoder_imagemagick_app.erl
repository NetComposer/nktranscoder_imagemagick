...
-module(nktranscoder_imagemagick_app).
...
-define(APP, nktranscoder_imagemagick).
...
start(_Type, _Args) ->
    ...
    case nklib_config:load_env(?APP, Syntax) of
        {ok, _} ->
            {ok, Pid} = nktranscoder_imagemagick_sup:start_link(),
            {ok, Vsn} = application:get_key(nktranscoder_imagemagick, vsn),
            lager:info("NkSERVER TRANSCODER ImageMagick v~s has started.", [Vsn]),
            {ok, Pid};
        {error, Error} ->
